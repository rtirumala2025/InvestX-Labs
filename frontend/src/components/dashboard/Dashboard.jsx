...
const Dashboard = () => {
  ...
  const {
    ...
    market: { data: marketData, loading: marketLoading, error: marketError },
    ai: {
      recommendations: aiRecommendations,
      loading: aiLoading,
      error: aiError,
    },
    mcp: { context: mcpContext, loading: mcpLoading, error: mcpError },
  ...
  const { isLoading, hasError, errorMessage } = useMemo(
    () => ({
      ...
      errorMessage:
        (portfolioError || marketError || aiError || mcpError)?.message ||
  ...
  const renderContent = useMemo(
    () => (
      ...
      >
        ...
        >
          <p>
            Data is automatically refreshed every 5 minutes. Last updated:{" "}
            {lastRefreshed?.toLocaleString() || "Never"}
          </p>
          {hasError && (
